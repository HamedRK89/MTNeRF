...
class NeRF_Depth(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, 
        ...
        """ 
        Modified NeRF architecture for depth supervision
        """
        super(NeRF_Depth, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
        ...
        if self.use_viewdirs:
            ...
            # Modified alpha prediction with depth-aware initialization
            self.alpha_linear = nn.Linear(W, 1)
            ...
            if self.depth_supervision:
                ...
                # Initialize for better depth convergence
                nn.init.constant_(self.alpha_linear.bias, -1.0)
                nn.init.normal_(self.alpha_linear.weight, mean=0.0, std=0.01)
                nn.init.constant_(self.depth_linear[-1].bias, 0.1)
                nn.init.normal_(self.depth_linear[-1].weight, mean=0.0, std=0.1)
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            if self.depth_supervision:
                ...
                depth_feat = self.depth_linear(h)
            
            # View direction processing
            h = torch.cat([feature, input_views], -1)
            for i, l in enumerate(self.views_linears):
                h = self.views_linears[i](h)
                h = F.relu(h)
            ...
            if self.depth_supervision:
                outputs = torch.cat([rgb, alpha, depth_feat], -1)
            else:
                outputs = torch.cat([rgb, alpha], -1)
        else:
            outputs = self.output_linear(h)
